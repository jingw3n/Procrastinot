...
import { Plus, Download, Search, ChevronDown, Filter, BarChart2, Calendar, AlertCircle, ClipboardList } from 'lucide-react';
...
export default function Assignments({ navigate }) {
  ...
  return (
    <div style={{ padding: isMobile ? '20px 16px' : '36px 40px' }}>
      ...
      {/* Tabs + Source filter */}
      <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'stretch' : 'center', gap: isMobile ? 10 : 0, borderBottom: '1px solid var(--border)', marginBottom: 20, paddingBottom: isMobile ? 10 : 0 }}>
        <div style={{ display: 'flex', gap: 0, overflowX: isMobile ? 'auto' : 'visible' }}>
          {TABS.map(tab => (
            <button
              key={tab}
        ...
        <div style={{ position: 'relative', width: isMobile ? '100%' : 160, marginBottom: isMobile ? 0 : 6 }}>
          <div
          ...
          {showSourceDropdown && (
            <div style={{ position: 'absolute', top: '100%', right: 0, width: isMobile ? '100%' : 160, background: '#fff', border: '1px solid var(--border)', borderRadius: 7, zIndex: 10, boxShadow: 'var(--shadow-sm)' }}>
